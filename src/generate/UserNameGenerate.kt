...
class UserNameGenerate(private val mRandom: Random = Random.Default) : IGenerate {
    companion object {
    ...
    private fun getChineseName(): Pair<String, Int> {
        var index: Int = getNum(0, firstName.length - 1)
        val first = firstName.substring(index, index + 1)
        val sex: Int = getNum(0, 1)
        var str = boy
        var length: Int = boy.length
        // 女
        if (sex == 0) {
            str = girl
            length = girl.length
        ...
        return Pair(first + second + third, sex)
    }
}